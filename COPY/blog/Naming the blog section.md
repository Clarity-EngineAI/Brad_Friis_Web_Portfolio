...
**Status:** chosen — **Field notes & stories**, Brad's decision, 17 August 2026
**Governs:** the eyebrow pill above the homepage blog teaser, the `/blog/` page headline, the nav
label, and the call to action. Four places, one name.
...
### 1.1 Decision — 17 August 2026

**Chosen reader-facing name: Field notes & stories.**

The choice resolves the two jobs the section has to do:

- **Field notes** carries practical business insight gathered through doing the work — including
  software personalisation, website journey mapping, design practice, sales, difficult customers,
  account relationships and trust with employees.
- **Stories** makes room for personal narrative, including the Cubs uniform and plastic bags, without
  making those pieces look like exceptions to an otherwise technical publication.

Together they name a body of writing rather than another capability or career heading, so the link
stands apart from the CV while remaining credible to the same hiring reader.

Use the exact reader-facing form **Field notes & stories**, in sentence case with the ampersand, in
all four places:

- Eyebrow pill: `Field notes & stories`
- `/blog/` page headline: `Field notes & stories`
- Navigation label: `Field notes & stories`
- Homepage call to action: `Explore field notes & stories`

The internal route and folder names remain `/blog/` and `COPY/blog/`.

---

## 2. The twelve options

These were banked before the 17 August decision and remain here as the options considered. None was
chosen. Grouped by register.
...
**Completed 17 August 2026.** After several naming rounds, Brad chose **Field notes & stories**. The
decision and its reasoning are recorded in §1.1; the earlier options remain in §2 as project history.

**Implemented 17 August 2026** (same day, later session — the decision sat unshipped for a few hours
first, surfaced when Brad asked why it seemed to have been lost). Updated: nav label
(`SiteLayout.astro:42,55`), `/blog/` eyebrow pill and `<title>` (`src/pages/blog/index.astro`).
`COPY/README.md` updated to record the 15 August "Brad calls it the blog" note as superseded. The
internal `/blog/` route and `COPY/blog/` folder were left unchanged, per this section. The homepage
call-to-action form ("Explore field notes & stories") has no live home yet — the homepage currently
has no blog teaser to update; apply it if one is added back.
