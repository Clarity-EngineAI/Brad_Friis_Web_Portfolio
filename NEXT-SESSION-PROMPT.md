...
**Updated:** Thursday 13 August 2026, 6:05 PM NZST
**Status:** Bespoke Astro static site. Framer and the Prolens template are abandoned. The build passes
with zero errors, warnings and hints across 10 pages, and all six published references carry a
redacted scan, all generated by `scripts/prepare-scans.py` from the full-resolution originals. Nothing
is known-broken; the site has not yet been reviewed in a real browser.
...
- **The transcriptions in `Brad Friis Resumes/References/` still spell the CanWest referee "Phillip".** It is Philip Lemon, one L, confirmed against the signature. `src/` is corrected; those files are not.
